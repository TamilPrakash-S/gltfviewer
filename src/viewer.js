  ...
  load ( url, rootPath, assetMap ) {
    ...
    return new Promise((resolve, reject) => {
      ...
      loader.load(url, (gltf) => {
        ...
        let bones = [];
        scene.traverse( n => {if(n.name.includes("Bone")) bones.push(n)});
        window.VIEWER.skeleton = new Skeleton(bones);
        window.VIEWER.rotateBone = (bone) => {
          // Update the bone's transformation here as desired.
          // For example, to rotate the bone:
          bone.position.x += 0.01;
          bone.position.y += 0.01;
          bone.position.z += 0.01;
        
          // Update all dependent bones (children).
          const children = bone.children;
          for (let i = 0; i < children.length; i++) {
            const childBone = children[i];
            window.VIEWER.rotateBone(childBone);
          }
        }
